...
test('Instructions issued (other than place) are ignored without a robot', () => {
  ...
  expect(
    simulate(boardState, Instruction.rotate(T.RotationalDirection.Left))[0]
  ).toEqual(boardState);
  expect(
    simulate(boardState, Instruction.rotate(T.RotationalDirection.Right))[0]
  ).toEqual(boardState);
...
test('The robot moves in the direction it is facing', () => {
  ...
  directionData.forEach(([direction, from, to]) => {
    const [updatedBoardState, effectFn] = simulate(
      {
        robot: { x: from[0], y: from[1], direction },
        board: GameBoard.defaultBoard()
      },
      Instruction.move()
    );
...
test('The robot rotates', () => {
  const directionData = [
    [
      ...
      T.CardinalDirection.West,
      T.CardinalDirection.East
    ],
    [
      T.CardinalDirection.East,
      T.CardinalDirection.North,
      T.CardinalDirection.South
    ],
    [
      T.CardinalDirection.South,
      T.CardinalDirection.East,
